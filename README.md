...
#### Prerequisites
1. Claim your $50 GCP credits using the link provided by instructor
2. Create a GCP project: `cs4xx-hw2-[yourusername]`
...
#### Deploy Instances
```bash
# Create 3 instances (costs ~$0.01/hour total with preemptible)
for i in 0 1 2; do
...
```bash
# Stop instances when not using (no charges while stopped)
...
1. Request nodes at https://cloudlab.us
2. SSH to each node
3. Clone repository and run services:
```bash
...
1. **Check assignment page** for full requirements
2. **Office Hours**: [Schedule]
3. **Piazza/Discord**: For questions (don't share code)
4. **GitHub Issues**: For starter code problems
